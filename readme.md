...
### 📁 `senana/`
Серверная часть для аналитики, включающая:
- **`langchain/`** — модуль для работы с LLM через Ollama
- Управление моделями и конфигурацией
- Обработка данных и пайплайны анализа
...
### Бизнес-пользователи:
- **Продуктовые менеджеры**: Понимание болевых точек пользователей и запросов на функции
- **Маркетинговые команды**: Оценка общественного восприятия кампаний и имиджа бренда
- **Менеджеры службы поддержки**: Выявление повторяющихся проблем и улучшение качества поддержки

### Разработчики:
- Интеграция возможностей анализа тональности в собственные приложения через REST API
